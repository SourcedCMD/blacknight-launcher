/* =========================================================================
   Inline icon set. Stroke-based, 24x24, currentColor - no icon font, no
   network requests, and every glyph inherits the accent theme for free.
   ========================================================================= */
(function () {
  'use strict';
  ...
  const P = {
    ...
    restore: '<rect x="8" y="3" width="13" height="13" rx="2"/><path d="M16 19a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V9a2 2 0 0 1 2-2"/>',
    ...
    store: '<path d="M3 9l1.5-5h15L21 9"/><path d="M3 9h18v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><path d="M8 9v3a4 4 0 0 0 8 0V9"/>',
    crown: '<path d="M2 18h20l-1.5-9-4.5 4L12 4 8 13 3.5 9z"/>',
    ...
    sparkles: '<path d="M12 3l1.9 5.6L19.5 10l-5.6 1.9L12 17.5l-1.9-5.6L4.5 10l5.6-1.4z"/><path d="M18 4l.7 2 2 .7-2 .7L18 9.4l-.7-2-2-.7 2-.7z"/>',
  ...
  /** Returns an SVG string for an icon name. */
  function icon(name, attrs = '') {
    const body = P[name];
    if (!body) return '';
    return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" ${attrs}>${body}</svg>`;
  }

  /** Same icon, but filled rather than stroked (used for play glyphs, stars). */
  function iconFilled(name, attrs = '') {
    const body = P[name];
    if (!body) return '';
    return `<svg viewBox="0 0 24 24" fill="currentColor" stroke="none" aria-hidden="true" ${attrs}>${body}</svg>`;
  }

  BN.icon = icon;
  BN.iconFilled = iconFilled;
  BN.iconNames = Object.keys(P);
})();
